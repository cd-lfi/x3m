    ...
    while True:
        if cap.isOpened():
            try:
                ret, frame = cap.read()
                if keep:
                    cv.imwrite(f"IM{time.time():.3f}_rtsp.png", frame)
                else:
...
def test_imshow(frames=5, keep=False):
    while True:
        ...
        if keep:
            cv.imwrite(f"IM{time.time():.3f}_test.png", frame)
        else:
            cv.imshow("Image", frame)
...
if __name__ == "__main__":
    ...
    if task == "rtsp":
        pull_rtsp(rtsp_url, frames=500, keep=False)
    else:
        test_imshow(frames=500, keep=False)
